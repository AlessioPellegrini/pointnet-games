/* ============================================================
   POINTNET MUSIC PLAYER (pointnet-music-player.js)
   Standalone, modular HTML5 audio player for PointNet Games.
   - Interactive progress bar with scrubbing/seek support
   - Live timer (currentTime / duration) formatted in m:ss
   - Track durations auto-detected and shown in playlist menu
   - Navigation: prev, play/pause, next, shuffle, volume
...
(function (root, factory) {
	...
	function PointNetMusicPlayer(options) {
		options = options || {};
		this.storageKey = options.storageKey || 'pointnet_music_player_prefs';
		...
		this.trackDurations = {}; // cache of loaded durations in seconds
		...
		this.playlist.forEach(function (tr) {
			...
			try {
				...
				probe.addEventListener('error', function () {
					/* Silently ignore 404/unsupported tracks in duration probe */
				});
			} catch (e) {}
		});
	};
