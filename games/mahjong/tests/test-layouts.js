...
const load = [
  ...
  '      // v0.9.2 — nessuna coordinata (z,x,y) duplicata (es. lyre)',
  '      const seen = {};',
  '      const dup = [];',
  ...
  '      });',
  '      if (bad.length || oddX.length || dup.length) {',
  '        failures++;',
  '        console.log("  FAIL " + name + "/" + variant + ": floaters=" + bad.length + " oddX=" + oddX.length + " dups=" + dup.length + (dup.length ? " " + dup.join(" ") : ""));',
  '      }',
  '    });',
  '  });',
